...
ETB.tabs = (function () {
  ...
  var STYLES = [
    ...
    '}',
    'html[data-etb-light]{',
      '--etb-bg:#FAF9F5;--etb-s1:#fff;--etb-s2:#fff;--etb-s3:#F5F3EC;',
    ...
    '}',
    // У КАЖДОЙ ЗОНЫ ЭКРАНА ОДИН ХОЗЯИН (находка Эллы 30.07). Панель живёт на
    // z-index 2147483635, оверлей витрины — на 2147483632, то есть панель всегда
    // сверху и при открытой витрине не пряталась: два слоя управления на одном
    // экране, один поверх другого. Спасала случайность — центр шапки витрины пуст,
    // а панель можно оттащить; любой элемент, добавленный в центр, немедленно
    // оказался бы под ней. Пока витрина открыта, панель уходит: она принадлежит
    // окну приложения, а не поверхности поверх него.
    'html[data-etb-mkt-open] #_etbv2_nav_center{',
      'opacity:0;pointer-events:none;transition:opacity .12s ease;',
    '}',
    '._etbv2_sec{',
      'display:flex;align-items:center;gap:4px;',
      'padding:8px 12px;border-radius:12px;cursor:pointer;',
      'font-size:13px;font-weight:500;color:var(--etb-tx2);',
    ...
    '#_etbv2_nav_grip{',
      ...
      'font-size:11px;letter-spacing:1px;user-select:none;flex-shrink:0;',
    '}',
