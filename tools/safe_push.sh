...
# Битые числовые HTML-сущности: 29.07 массовая замена цветов съела &#128269;
# (лупа) — «#128269» подошло под шаблон hex-цвета и стало «&#24544F;», в интерфейсе
# вылезли иероглифы. Проверка дешёвая, класс ошибки противный.
BAD_ENT=$(grep -o "&#[0-9]*[A-Fa-f][0-9A-Fa-f]*;" toolbar/public/plugins_manager.html | sort -u || true)
if [ -n "$BAD_ENT" ]; then
  say "Останавливаюсь: битые HTML-сущности (буквы в числовой ссылке):"
  echo "$BAD_ENT"
  exit 1
fi
...
GATES_FAILED=""
for G in check-account-scope check-runtime-portability check-reproducible-build check-catalog-contract check-ru-plurals check-ui-hardcoded-text; do
