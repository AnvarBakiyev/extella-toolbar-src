...
ETB.router = (function () {
  ...
  function _evolutionNormalizeLabReadiness(raw, session, context) {
    var keys = [
      ...
      'target_resolution', 'owner_device_access', 'single_use'
    ];
  ...
  function _resolveDeviceIdFromAccount() {
    ...
    return ETB.api.kvGet('_device_id').catch(function (e) {
      ...
      // След на аккаунте: что именно случилось при поиске устройства. DevTools в
      // проде выключены, у коллег их тем более нет — без этого следа каждая
      // поломка «панель не знает устройство» стоит круга «поставь — перезапусти —
      // пришли экран». Пишем и на успехе (каким путём нашли), и на отказе.
      try {
        ETB.api.kvSet('_device_probe',
          (dev ? 'нашлось: ' + dev : 'не нашлось') +
          ' · ' + (_deviceWhy || 'сразу от приложения') +
          ' · ' + new Date().toISOString(),
          'Диагностика поиска устройства (витрина)').catch(function () {});
      } catch (_) {}
      return dev;
    });
  }
