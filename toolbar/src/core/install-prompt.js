...
ETB.installPrompt = (function () {
  ...
  function build(ctx, analysis) {
    var lines = [
      ...
      'Any UI you put in install_dir/index.html is iframed by the toolbar, which postMessages it:',
      '  { type: "etb_init", pluginId, token, apiBase: "https://api.extella.ai", experts: [...] }',
      'To perform a real device action from a button, call an Extella expert over REST:',
      '  window.addEventListener("message", function(e){',
      '    if (e.data && e.data.type === "etb_init") { window.__tok = e.data.token; window.__api = e.data.apiBase; }',
      '  });',
      '  async function runExpert(name, params){',
      '    const r = await fetch(window.__api + "/api/expert/run", { method:"POST",',
      '      headers:{ "Content-Type":"application/json", "X-Auth-Token":window.__tok,',
      '        "X-Profile-Id":"default", "X-Agent-Id":"agent_extella_default" },',
      '      body: JSON.stringify({ expert_name:name, params:params }) });',
      '    const d = await r.json(); return d.result !== undefined ? d.result : d;',
      '  }',
      ...
      'The ONLY network call it may make is runExpert() -> /api/expert/run shown above (or, for a',
      'local_server plugin, its own http_port). NEVER invent or fetch any other endpoint — no',
      '/api/upload, no /api/ocr, no /predict, no custom API. Those return an HTML 404 page, so the',
      'UI dies with "Unexpected token \'<\'" and "Failed to fetch". There is NO file-upload endpoint.',
      ...
      '3) Create any Extella experts you need (categories 2 & 3) and save them.',
      ...
      '   for 1a/2/3, the real service port for 1b). Poll GET http://localhost:<SERVE_PORT>/ as follows:',
      '   Base poll: 12 retries × 5 s = 60 s max. If the server has not responded yet, read its log file',
      '   (if you wrote one via nohup/the server process). If the log contains words like "downloading",',
      '   "compiling", "building", "Installing", "Downloading" or similar — the server is still starting up:',
      '   extend the wait by one extra round of 12 retries × 5 s (another 60 s). If after that it still',
      '   does not respond, STOP polling and move on to diagnose + fix rather than waiting further.',
  ...
  function buildRepair(plugin, failure) {
    ...
    var lines = [
      ...
      'X-Agent-Id:"agent_extella_default"; body {expert_name, params}) to run real action experts',
    ...
    var lines = [
      ...
      fullReset
        ? [
            ...
            '1b. Kill any process on port ' + port + ':',
            '      import subprocess',
            '      subprocess.run(["bash","-c","lsof -ti:' + port + ' | xargs kill -9 2>/dev/null || true"])',
          ].join('\n')
        : [
